...
# > varies by user
#if(file.exists("~/Dropbox/Lagged-Precip/Data/")){
  ## 
...
datfiles <- list.files(paste0(datpath, "Soil-Moisture/Mega-Comp_soil-moisture/Mega-Comp_soil-moisture_2022-02-23_combined"), 
...
logger_plot <- soilmoisture_all %>%
    filter(!is.na(vwc)) %>%
    ggplot(aes(dowy, vwc, color = treatment, 
  ...
  print(logger_plot)
  ggsave(filename = paste0(datpath, "Soil-Moisture/Mega-Comp_soil-moisture/Mega-Comp_rawVWC_2022-02-23.pdf"),
         plot = logger_plot,
